...
function LaptopComputerCategory() {
    const { subcat } = useParams();
    const laptopComputerProductFilter = LaptopComputerData.filter((item) => (item.productCat === subcat));
    return (
        <>
            <section>
                <div className="container my-5">
                    <h3 className='text-uppercase'>{subcat}</h3>
                    {laptopComputerProductFilter && (
                        <div className="row">
                            {laptopComputerProductFilter.map((item) => (
...
export default LaptopComputerCategory
